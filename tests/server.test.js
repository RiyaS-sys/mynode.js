...
describe('Blood Bank API Tests', () => {
  ...
  // Test donor registration endpoint
  test('POST /submit-form - should register a new donor', async () => {
    const res = await request(app)
      .post('/submit-form')
      .send({
    ...
    expect(res.status).toBe(200);
    expect(res.text).toBe('Donor registration successful!');
  });

  // Test admin login endpoint
  test('POST /admin/login - should authenticate admin successfully', async () => {
    const res = await request(app)
      .post('/admin/login')
      .send({
        username: 'admin',
  ...
  test('GET /donors - should return donors list', async () => {
    const res = await request(app)
      .get('/donors');
  ...
  // Test get blood requests endpoint
  test('GET /blood-requests - should return blood requests', async () => {
    const res = await request(app)
      .get('/blood-requests');
